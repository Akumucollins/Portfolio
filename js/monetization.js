var _bsa = {
  init: function (format, zoneKey, segment, options) {
    ...
    if (
      ...
      if (segment)
        jsonSrc = this.appendQueryString(jsonSrc, "segment", segment);
      if (options && this.isset(options.ip))
        jsonSrc = this.appendQueryString(jsonSrc, "forwardedip", options.ip);
      if (options && this.isset(options.country))
        jsonSrc = this.appendQueryString(jsonSrc, "country", options.country);
      if (options && this.isset(options.number_of_ads))
        jsonSrc = this.appendQueryString(
          jsonSrc,
          "forcenads",
          options.number_of_ads
        );
      if (forcebanner)
        jsonSrc = this.appendQueryString(jsonSrc, "forcebanner", forcebanner);
      ...
      if (forwardedip)
        jsonSrc = this.appendQueryString(jsonSrc, "forwardedip", forwardedip);
...
_bsa.srvConv = function (reqHash, typeName) {
...
_bsa.trackConv = function () {
  ...
  if (landed) {
    ...
    if (reqHash) {
      _bsa.setCookie("_bsap_landed", reqHash, 1000 * 60 * 15);
...
_bsa.default = function (zoneKey, segment, options, ads) {
  var disable_css =
    _bsa.isset(options) && _bsa.isset(options.disable_css)
      ? options.disable_css
      : false;
  var custom_domain =
    _bsa.isset(options) && _bsa.isset(options.custom_domain)
      ...
      : false;
  var image_type =
    _bsa.isset(options) && _bsa.isset(options.image) ? options.image : false;
  ...
  function template(ads) {
    var c =
      '<span class="default-ad">' +
      (_bsa.isset(options) && _bsa.isset(options.default_text)
    ...
    for (var i = 0; i < ads.length; i++) {
      c +=
        ...
        (_bsa.isset(ads[i].base64)
          ? ads[i].base64
          : image_type == "logo"
          ? ads[i].logo
          : _bsa.isset(ads[i].image)
          ? ads[i].image
          : ads[i].logo) +
        '" style="background: ' +
  ...
  if (ads) {
    _bsa.drop(
      (disable_css ? "" : css(ads)) + template(ads),
      options.target,
      "div",
      _bsa.default.elID(options),
      _bsa.default.attributes
    );
...
_bsa.defaultcta = function (zoneKey, segment, options, ads) {
  ...
  function css(colors) {
    ...
    c +=
      ...
      (_bsa.isset(options) && _bsa.isset(options.heading_background)
        ? options.heading_background
        : "#000000") +
      ";color:" +
      (_bsa.isset(options) && _bsa.isset(options.heading_text)
      ...
      (_bsa.isset(options) && _bsa.isset(options.cta_background)
        ? options.cta_background
        : "#000000") +
      ";color:" +
      (_bsa.isset(options) && _bsa.isset(options.cta_text)
        ? options.cta_text
        : "#000000") +
      ";}#" +
      id +
...
_bsa.imageonly = function (zoneKey, segment, options, ads) {
  ...
  function css(ads) {
    ...
    if (_bsa.imageonly.align(options) == "vertical")
      c +=
        "#" +
        id +
        "{flex-direction:column}#" +
        id +
        " ._imageonly_{margin-bottom:10px;}";
    else
      c +=
        "#" + id + "{flex-direction:row}#" + id + " a._imageonly_{width:49%;}";
    if (ads.length <= 1)
      c +=
        "#" +
        id +
  ...
  function template(ads) {
    ...
    if (ads.length <= 1)
      c +=
        '<a href="' +
        ads[0].ad_via_link +
        '" target="_blank" class="imageonly-ad">' +
        (_bsa.isset(options) && _bsa.isset(options.default_text)
          ? options.default_text
          : "ad") +
        "</a>";
    for (var i = 0; i < ads.length; i++) {
      if (_bsa.isset(ads[i].snippet))
        c +=
          '<span class="_imageonly_ ad' +
          i +
          " " +
          ((i + 1) % 2 ? "odd" : "even") +
          '">' +
          ads[i].snippet.replace(
            "https://cdn4.buysellads.net/backfill/404/45268.html?t=1599250072",
            "http://custom.src/test/carbon-backfill.html"
          ) +
          "</span>";
      else
        c +=
...
_bsa.fancybar = function (zoneKey, segment, options, ads) {
  ...
  function css(colors, show_ads_via) {
    var c = colors[0],
      backgroundColor = _bsa.isset(c.backgroundColor)
        ? c.backgroundColor
        : _bsa.testData.ads[0].backgroundColor,
      backgroundHoverColor = _bsa.isset(c.backgroundHoverColor)
        ? c.backgroundHoverColor
        : _bsa.testData.ads[0].backgroundHoverColor,
      callToActionTextColor = _bsa.isset(c.ctaTextColor)
        ? c.ctaTextColor
        : _bsa.testData.ads[0].ctaTextColor,
      callToActionTextColorHover = _bsa.isset(c.ctaTextColorHover)
        ? c.ctaTextColorHover
        : _bsa.testData.ads[0].ctaTextColorHover,
      ctaBackgroundColor = _bsa.isset(c.ctaBackgroundColor)
        ? c.ctaBackgroundColor
        : _bsa.testData.ads[0].ctaBackgroundColor,
      ctaBackgroundHoverColor = _bsa.isset(c.ctaBackgroundHoverColor)
        ? c.ctaBackgroundHoverColor
        : _bsa.testData.ads[0].ctaBackgroundHoverColor,
      textColor = _bsa.isset(c.textColor)
        ? c.textColor
        : _bsa.testData.ads[0].textColor,
      textColorHover = _bsa.isset(c.textColorHover)
        ? c.textColorHover
        : _bsa.testData.ads[0].textColorHover,
    ...
    return (
      ...
      id +
      " .fancybar-disclaimer{font-size:10px}}</style>"
    );
  }
  function template(ads) {
    var a = ads[0],
      adViaLink = _bsa.isset(a.ad_via_link)
        ? _bsa.htmlEncode(a.ad_via_link)
        : "https://buysellads.com/",
      callToAction = _bsa.isset(a.callToAction)
        ? a.callToAction
        : _bsa.testData.ads[0].callToAction,
      company = _bsa.isset(a.heading) ? a.heading : a.company,
      description = _bsa.isset(a.description)
        ? a.description
        : _bsa.testData.ads[0].description,
      logo = _bsa.isset(a.base64)
        ? a.base64
        : _bsa.isset(a.logo)
        ? a.logo
        : _bsa.isset(a.transparentLogo)
        ? a.transparentLogo
        : a.image,
    ...
    c +=
      ...
      "</span>";
    if (show_ads_via && tagline.length > 30)
      c +=
        '<span class="fancybar-disclaimer" title="' +
        _bsa.htmlEncode(tagline) +
        '">' +
        _bsa.htmlEncode(tagline) +
        "</span>";
    c +=
      "</span></span></span>" +
      '<span class="fancybar-right"><span class="fancybar-cta">' +
      _bsa.htmlEncode(callToAction) +
      "</span></span></a>";
    c +=
      '<div class="fancybar-footer"><span class="fancybar-close" onclick="_bsa.close(\'' +
      _bsa.fancybar.elID(options) +
      '\')">Hide Ad</span> &bull; <a class="fancybar-via" href="' +
      adViaLink +
  ...
  if (ads) {
    _bsa.drop(
      (disable_css ? "" : css(ads, show_ads_via)) + template(ads),
      "body",
      "div",
      _bsa.fancybar.elID(options),
      _bsa.fancybar.attributes
...
_bsa.flexbar = function (zoneKey, segment, options, ads) {
  ...
  function css(colors) {
    var c = colors[0],
      backgroundColor =
        _bsa.isset(c) && _bsa.isHex(c.barcolor)
          ? c.barcolor
          : _bsa.isset(c.backgroundColor)
          ? c.backgroundColor
          : _bsa.testData.ads[0].backgroundColor,
      backgroundHoverColor =
        _bsa.isset(c) && _bsa.isHex(c.barcolorHover)
          ...
          : _bsa.testData.ads[0].backgroundHoverColor,
      callToActionTextColor =
        _bsa.isset(c) && _bsa.isHex(c.callToActionTextColor)
          ? c.callToActionTextColor
          : _bsa.isset(c.ctaTextColor)
          ? c.ctaTextColor
          : _bsa.testData.ads[0].callToActionTextColor,
      callToActionTextColorHover =
        _bsa.isset(c) && _bsa.isHex(c.callToActionTextColorHover)
          ? c.callToActionTextColorHover
          : _bsa.isset(c.ctaTextColorHover)
          ? c.ctaTextColorHover
          : _bsa.testData.ads[0].callToActionTextColorHover,
      ctaBackgroundColor =
        _bsa.isset(c) && _bsa.isHex(c.callToActionColor)
          ...
          : _bsa.testData.ads[0].ctaBackgroundColor,
      ctaBackgroundHoverColor =
        _bsa.isset(c) && _bsa.isHex(c.callToActionColorHover)
          ? c.callToActionColorHover
          : _bsa.isset(c.ctaBackgroundHoverColor)
          ? c.ctaBackgroundHoverColor
          : _bsa.testData.ads[0].ctaBackgroundHoverColor,
      textColor =
        _bsa.isset(c) && _bsa.isHex(c.textcolor)
          ? c.textcolor
          : _bsa.isset(c.textColor)
          ? c.textColor
          : _bsa.testData.ads[0].textColor,
      textColorHover =
        _bsa.isset(c) && _bsa.isHex(c.textcolorHover)
          ? c.textcolorHover
          : _bsa.testData.ads[0].textColorHover,
    ...
    return (
      "<style>#" +
      id +
      "._bsa_hide ._bsa_flexbar{display:none}#" +
      id +
      "._bsa_show ._bsa_flexbar{display:block}#" +
      id +
      "{display:block}#" +
      id +
      "._bsa_hide{margin-bottom:0}#" +
      id +
      " ._bsa_flexbar{position:relative;z-index:999999;width:100%;background:" +
      backgroundColor +
      ';text-align:left;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Oxygen-Sans,Ubuntu,Cantarell,"Helvetica Neue",Helvetica,Arial,sans-serif !important;-webkit-animation:fadein 2s;-moz-animation:fadein 2s;-ms-animation:fadein 2s;-o-animation:fadein 2s;animation:fadein 2s}#' +
      id +
      " ._bsa_flexbar:hover{background:" +
      backgroundHoverColor +
      "}#" +
      id +
      " .flexbar-wrap{display:flex;padding:18px 40px;text-decoration:none;align-items:center;justify-content:space-between}#" +
      id +
      " .flexbar-left{display:flex;align-items:center;justify-content:flex-start}#" +
      id +
      " .flexbar-text{display:flex;flex-direction:column;margin-right:60px}#" +
      id +
      " .flexbar-heading{color:" +
      textColor +
      ";text-transform:uppercase;letter-spacing:1px;font-weight:600;font-size:10px}#" +
      id +
      " .flexbar-sponsor{letter-spacing:1px;font-weight:400;opacity:.8}#" +
      id +
      " .flexbar-desc{color:" +
      textColor +
      ";letter-spacing:1px;font-size:16px;font-weight:400;line-height:1.3}#" +
      id +
      " .flexbar-logo{line-height:0}#" +
      id +
      " .flexbar-logo img{margin:0 30px 0 0;max-width:125px}#" +
      id +
      " .flexbar-cta{padding:10px 14px;border:0;border-radius:3px;background:" +
      ctaBackgroundColor +
      ";color:" +
      callToActionTextColor +
      ";text-decoration:none;text-transform:uppercase;white-space:nowrap;letter-spacing:.5px;font-weight:600;font-size:14px;line-height:1}#" +
      id +
      " .flexbar-footer{position:absolute;right:40px;bottom:4px;border-top:0;line-height:1;border-top-left-radius:3px;color:" +
      textColor +
      ";text-decoration:none;text-transform:uppercase;letter-spacing:1px;font-weight:600;font-size:8px}#" +
      id +
      " .flexbar-close{color:" +
      textColor +
      "cc;cursor:pointer}#" +
      id +
      " .flexbar-close:hover{color:" +
      textColor +
      "}#" +
      id +
      " .flexbar-via{color:" +
      textColor +
      "cc !important;text-decoration:none !important}#" +
      id +
      " .flexbar-via:hover{color:" +
      textColor +
      " !important}@media all and (max-width:768px){#" +
      id +
      " .flexbar-logo img{max-width:100px}#" +
      id +
      " .flexbar-heading{font-size:9px;line-height:1.5;margin-bottom:2px}#" +
      id +
      " .flexbar-desc{font-size:14px}#" +
      id +
      " .flexbar-cta{font-size:12px}}@media all and (max-width:568px){#" +
      id +
      " .flexbar-logo img{display:none}#" +
      id +
      " .flexbar-cta{display:none}#" +
      id +
      " .flexbar-heading{font-size:8px}#" +
      id +
      " .flexbar-desc{font-size:12px}#" +
      id +
      " .flexbar-text{margin-right:0}#" +
      id +
      " .flexbar-footer{right:15px}#" +
      id +
      " .flexbar-wrap{padding:20px 15px}}</style>"
    );
  }
  function template(ads) {
    var a = ads[0],
      ...
      callToAction = _bsa.isset(a.callToAction)
        ? a.callToAction
        : _bsa.testData.ads[0].callToAction,
      company = _bsa.isset(a.company)
        ? a.company
        : _bsa.testData.ads[0].company,
      description = _bsa.isset(a.description)
        ? a.description
        : _bsa.testData.ads[0].description,
      logo = _bsa.isset(a.base64)
  ...
  if (ads) {
    for (var i = 0; i < ads.length; i++) {
      ...
      _bsa.drop(
        ...
        "div",
        _bsa.recommended.elID(options) + i,
        _bsa.recommended.attributes
      );
    ...
    for (var i = 0; i < ads.length; i++) {
      ...
      c[i] += single[i]
        ...
        .replace(
          ...
          _bsa.htmlEncode(ads[i].backgroundColor)
        )
        .replace(
          new RegExp("##backgroundHoverColor##", "g"),
          _bsa.htmlEncode(ads[i].backgroundHoverColor)
        )
        .replace(
          new RegExp("##ctaBackgroundColor##", "g"),
          _bsa.htmlEncode(ads[i].ctaBackgroundColor)
        )
        .replace(
          new RegExp("##ctaBackgroundHoverColor##", "g"),
          _bsa.htmlEncode(ads[i].ctaBackgroundHoverColor)
        )
        .replace(
          new RegExp("##ctaTextColor##", "g"),
          _bsa.htmlEncode(ads[i].ctaTextColor)
        )
        .replace(
          new RegExp("##ctaTextColorHover##", "g"),
          _bsa.htmlEncode(ads[i].ctaTextColorHover)
        )
        .replace(
          new RegExp("##textColor##", "g"),
          _bsa.htmlEncode(ads[i].textColor)
        )
        .replace(
          new RegExp("##textColorHover##", "g"),
          _bsa.htmlEncode(ads[i].textColorHover)
        )
        .replace(
          new RegExp("##timestamp##", "g"),
          _bsa.htmlEncode(ads[i].timestamp)
        )
        .replace(
          new RegExp("##callToAction##", "g"),
          _bsa.htmlEncode(ads[i].callToAction)
        )
        .replace(
        ...
        .replace(
          new RegExp("##tagline##", "g"),
          _bsa.htmlEncode(ads[i].companyTagline)
        )
        .replace(
  ...
  function css(colors) {
    ...
    if (_bsa.stickybox.align(options) == "vertical")
      c +=
        ...
        "{flex-direction:column}#" +
        id +
        " ._stickybox_{margin-bottom:10px;}";
    else
      c +=
        "#" + id + "{flex-direction:row}#" + id + " a._stickybox_{width:49%;}";
    c +=
      "#" +
      id +
      "{flex-direction:column}#" +
      id +
      " ._stickybox_{margin:0}#" +
      id +
      "._bsa_hide{display:none}#" +
      id +
      "._bsa_show{display:flex}#" +
      id +
      "{position:relative;position:fixed;right:20px;bottom:20px;z-index:999 !important;display:flex;max-width:360px;border:1px solid #eaeaea;border-radius:2px;background:rgba(255,255,255,.95);box-shadow:rgba(0,0,0,.10) 1px 1px 15px 0;-webkit-animation:fadein .85s;-moz-animation:fadein .85s;-ms-animation:fadein .85s;-o-animation:fadein .85s;animation:fadein .85s}#" +
      id +
      " a._stickybox_{position:relative;display:flex;padding:10px 20px 10px 10px;text-decoration:none}#" +
      id +
      " a._stickybox_:hover .stickybox-title{text-decoration:underline}#" +
      id +
      " .stickybox-ad{position:absolute;bottom:57px;left:-18px;display:block;padding:2px 0;width:18px;border-radius:2px 0 0 2px;background:rgba(0,0,0,.1);color:#6e6e6e;text-align:center;text-decoration:none;font-size:11px}#" +
      id +
      " .stickybox-ad:hover{background:rgba(0,0,0,.2)}#" +
      id +
      " a._stickybox_ .stickybox-image{margin-right:10px;padding:10px;display:table;border-radius:2px;line-height:0}#" +
      id +
      " a._stickybox_ .stickybox-image img{max-width:40px}#" +
      id +
      " a._stickybox_ .stickybox-title{display:block;line-height:1.5;font-weight:600;font-size:14px}#" +
      id +
      " a._stickybox_ .stickybox-description{display:block;max-width:350px;color:#414141;line-height:1.35;font-size:13px}#" +
      id +
      " a._stickybox_:hover .stickybox-description{color:#111}#" +
      id +
      " .stickybox-close{position:absolute;top:7px;right:7px;display:block;float:right;width:18px;height:18px;border-radius:50px;background:rgba(0,0,0,.1);color:#fff;text-align:center;text-decoration:none;font-weight:bold;font-size:12px;line-height:150%}#" +
      id +
      ' .stickybox-close:hover{background:rgba(0,0,0,.3);color:rgba(255,255,255,.8);cursor:pointer}@keyframes "fadein"{from{opacity:0}to{opacity:1}}@-moz-keyframes "fadein"{from{opacity:0}to{opacity:1}}@-webkit-keyframes "fadein"{from{opacity:0}to{opacity:1}}@-ms-keyframes fadein{from{opacity:0}to{opacity:1}}@-o-keyframes fadein{from{opacity:0}to{opacity:1}}';
    c += "</style>";
    return c;
  }
  function template(ads) {
    var c =
      '<a href="' +
      ads[0].ad_via_link +
      '" rel="noopener sponsored" target="_blank" class="stickybox-ad">' +
      (_bsa.isset(options) && _bsa.isset(options.stickybox_text)
        ? options.stickybox_text
        : "ad") +
      "</a>";
    for (var i = 0; i < 1; i++) {
      var companyTagline = _bsa.isset(ads[i].companyTagline)
        ? ads[i].companyTagline
        : _bsa.isset(ads[i].company)
        ? ads[i].company
        : "";
      c +=
        '<a href="' +
        _bsa.link(ads[i].statlink, segment, custom_domain, ads[i].timestamp) +
        '" target="_blank" rel="nofollow noopener" class="_stickybox_"><span class="stickybox-image" style="background-color:' +
        ads[i].backgroundColor +
        '";><img src="' +
        (_bsa.isset(ads[i].base64) ? ads[i].base64 : ads[i].image) +
        '" /></span><div class="stickybox-text"><span class="stickybox-title">' +
        _bsa.htmlEncode(companyTagline) +
        '</span><span class="stickybox-description">' +
        _bsa.htmlEncode(ads[i].description) +
        '</span></div></a><span class="stickybox-close" onclick="_bsa.close(\'' +
        _bsa.stickybox.elID(options) +
        "')\">&#10005;</span>";
      c += _bsa.pixel(ads[i].pixel, ads[i].timestamp);
    }
    return c;
...
_bsa.hovercard = function (zoneKey, segment, options, ads) {
  ...
  function template(ads) {
    var c = "";
    for (var i = 0; i < ads.length; i++) {
      ads[i].companyTagline = ads[i].companyTagline
        ? ads[i].companyTagline
        : ads[i].company;
      c +=
        '<a href="' +
        ads[i].statlink +
        '" class="hover-section" target="_blank" rel="noopener sponsored">';
      c +=
        '<div class="hover-banner" style="background-color: ' +
        ads[i].backgroundColor +
        '">';
      c +=
        '<span style="color: ' +
        ...
        "; background-color: " +
        ads[i].textColor +
        'CC" class="hover-ad">Sponsored</span>';
      c += '<img class="hover-logo" src="' + ads[i].logo + '">';
      c += "</div>";
      c += '<div class="hover-subset">';
      c +=
        '<img class="hover-icon" style="background-color: ' +
        ads[i].backgroundColor +
        '" src="' +
        ads[i].image +
        '">';
      c +=
        '<div class="hover-description"><strong>' +
        ads[i].company +
        "</strong> " +
        ads[i].description +
        "</div>";
...
_bsa.iab = function (zoneKey, segment, options, ads) {
  ...
  function css(colors) {
    ...
    return (
      ...
      " .z_" +
      zoneKey +
      ' ._bsa_iab{display:flex;padding:20px 20px;text-decoration:none;border:solid 1px #e4e6e8;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Oxygen-Sans,Ubuntu,Cantarell,"Helvetica Neue",sans-serif;flex-flow:row nowrap;justify-content:space-between;align-items:center;background:-ms-linear-gradient(-30deg,' +
      backgroundColor +
      "," +
      backgroundColor +
      " 45%," +
      backgroundColor +
      " 45%);background:-webkit-linear-gradient(-30deg," +
      backgroundColor +
      "," +
      backgroundColor +
      " 45%," +
      backgroundColor +
      " 45%);background:linear-gradient(-30deg," +
      backgroundColor +
      "E5," +
      backgroundColor +
      "E5 45%," +
      backgroundColor +
      " 45%) #fff;box-sizing:border-box}#" +
      id +
      " .z_" +
      zoneKey +
      " ._bsa_iab.leaderboard{width:728px;height:90px}#" +
      id +
      " .z_" +
      zoneKey +
      " ._bsa_iab.rectangle{width:300px;height:250px}#" +
      id +
      " .z_" +
      zoneKey +
      " ._bsa_iab .iab-image{margin-right:20px;max-height:50px;border-radius:3px}#" +
      id +
      " .z_" +
      zoneKey +
      " ._bsa_iab.rectangle .iab-image{margin-bottom:10px}#" +
      id +
      " .z_" +
      zoneKey +
      " ._bsa_iab .iab-main{display:flex;flex-grow:1;flex-flow:row nowrap;align-items:center}#" +
      id +
      " .z_" +
      zoneKey +
      " ._bsa_iab.rectangle .iab-main{display:flex;flex-grow:1;flex-flow:wrap}#" +
      id +
      " .z_" +
      zoneKey +
      " ._bsa_iab .iab-details{display:block;margin-right:10px;color:" +
      textColor +
      " !important;flex-flow:column nowrap}#" +
      id +
      " .z_" +
      zoneKey +
      " ._bsa_iab .iab-company{margin-bottom:4px;text-transform:uppercase;letter-spacing:2px;font-size:11px}#" +
      id +
      " .z_" +
      zoneKey +
      " ._bsa_iab.rectangle .iab-company.iab-first{display:none}#" +
      id +
      " .z_" +
      zoneKey +
      " ._bsa_iab.leaderboard .iab-company.iab-aux{display:none}#" +
      id +
      " .z_" +
      zoneKey +
      " ._bsa_iab.rectangle .iab-company.iab-aux{color:" +
      textColor +
      " !important;margin-top:20px;opacity:.7}#" +
      id +
      " .z_" +
      zoneKey +
      " ._bsa_iab .iab-desc{letter-spacing:1px;font-weight:300;font-size:13px;line-height:1.4}#" +
      id +
      " .z_" +
      zoneKey +
      " ._bsa_iab.rectangle .iab-desc{font-size:15px}#" +
      id +
      " .z_" +
      zoneKey +
      " ._bsa_iab .iab-cta{padding:10px 14px;border-radius:3px;text-transform:uppercase;white-space:nowrap;letter-spacing:1px;font-weight:400;font-size:12px;transition:all .3s ease-in-out;color:" +
      callToActionTextColor +
      ";background-color:" +
      ctaBackgroundColor +
      ";margin-left:auto}#" +
      ...
      " .z_" +
      zoneKey +
      " ._bsa_iab.rectangle .iab-cta{margin-left:inherit;margin-top:15px}</style>"
    );
  }
  function template(ads) {
    var c = "";
    for (var i = 0; i < 1; i++) {
      c +=
        ...
        '" target="_blank" rel="noopener sopnsored" class="_bsa_iab ' +
        _bsa.iab.iabformat(options) +
        '"><span class="iab-main"><img class="iab-image" src="' +
        (_bsa.iab.iabformat(options) == "rectangle"
          ? ads[i].logo
          : ads[i].logo) +
        '" /><span class="iab-details"><div class="iab-company iab-first">Sponsored by <strong>' +
        _bsa.htmlEncode(ads[i].company) +
        '</strong></div><div class="iab-desc">' +
        _bsa.htmlEncode(ads[i].description) +
        '</div></span><span class="iab-cta">' +
        _bsa.htmlEncode(ads[i].callToAction) +
        '</span><span class="iab-company iab-aux">Sponsored by <strong>' +
        _bsa.htmlEncode(ads[i].company) +
...
_bsa.carbonbackfill = function (a) {
  ...
  carbon.src =
    "https://cdn.carbonads.com/carbon.js?serve=" + a.options.carbonfallback;
